...
class PythonServiceResource:
    ...
    def __init__(self):
        if SAGEMAKER_MULTI_MODEL_ENABLED:
            ...
            if prediction_service_pb2_grpc:
                for grpc_port in self._tfs_grpc_ports:
                    # Initialize channels here so gunicorn worker could have mapping between
                    # each grpc port and its own channel
                    self.get_prediction_service(grpc_port)
            else:
                log.warn("prediction_service_pb2_grpc unavailable since tensorflow serving "
                         "dependency is not imported, which grpc channel won't be created. "
                         "Please check if dependency exists in requirements.txt.")
                # Tensorflow serving dependency not imported so initialize mapped channel value
                # of each grpc port as None
                for grpc_port in self._tfs_grpc_ports:
                    self.prediction_service[grpc_port] = None
    ...
    def _handle_invocation_post(self, req, res, model_name=None):
        if SAGEMAKER_MULTI_MODEL_ENABLED:
            ...
            data, context = tfs_utils.parse_request(req, rest_port, grpc_port,
                                                self._tfs_default_model_name,
                                                channel = self.prediction_service[int(grpc_port)])
    ...
    def get_prediction_service(self, grpc_port):
        if grpc_port not in self.prediction_service:
            log.info("Creating prediction service for port: %s", grpc_port)
            channel = grpc.insecure_channel("localhost:{}".format(grpc_port))
            self.prediction_service[grpc_port] = prediction_service_pb2_grpc.PredictionServiceStub(channel)
